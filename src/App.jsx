...
import { Trophy, XCircle, ChevronRight, RefreshCw, Heart, User, CheckCircle2, Info, ChevronDown, ChevronUp } from 'lucide-react';

// Configuración de las preguntas (30 en total) - Tus preguntas personalizadas
const QUESTIONS = [
      ...
      <div className="max-w-md w-full bg-white rounded-3xl shadow-2xl overflow-hidden transition-all duration-300 my-8">
        ...
        {gameState === 'START' && (
          <div className="p-8 text-center animate-in fade-in zoom-in duration-500">
            ...
            <p className="text-slate-500 mb-8 leading-relaxed">¿Sabes todo sobre mí? Responde este test de {totalQuestions} preguntas.</p>
            <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 mb-8 text-sm text-amber-800 text-left">
              <p className="font-bold flex items-center gap-2 mb-1">
                <Trophy className="w-4 h-4 text-amber-600" /> Objetivo:
              </p>
              Debes acertar al menos {passingScore} respuestas para ganar.
            </div>
            <button onClick={startNameEntry} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl shadow-lg flex items-center justify-center gap-2 group transition-all">
              Continuar
        ...
        {gameState === 'NAME_ENTRY' && (
          <div className="p-8 animate-in slide-in-from-right duration-300">
            ...
            <p className="text-slate-500 mb-8">Dime tu nombre antes de empezar.</p>
            <div className="relative mb-8 text-left">
              ...
              <input
                ...
                className="block w-full pl-11 pr-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none text-lg"
                autoFocus
              />
            </div>
            <button onClick={startQuiz} disabled={userName.trim().length < 2} className={`w-full font-bold py-4 rounded-2xl shadow-lg transition-all flex items-center justify-center gap-2 ${userName.trim().length >= 2 ? 'bg-indigo-600 text-white' : 'bg-slate-200 text-slate-400 cursor-not-allowed'}`}>
              Comenzar Test
        ...
        {gameState === 'QUIZ' && (
          <div className="p-6 md:p-8 animate-in fade-in duration-300">
            <div className="flex justify-between items-end mb-6">
              ...
              <div className="w-24 h-2 bg-slate-100 rounded-full mb-1 overflow-hidden">
                <div className="h-full bg-indigo-500 transition-all duration-300" style={{ width: `${((currentQuestion + 1) / totalQuestions) * 100}%` }}></div>
              </div>
            </div>
            <div className="mb-8 min-h-[90px] flex items-center text-left">
              <p className="text-xl text-slate-800 font-semibold leading-tight">{QUESTIONS[currentQuestion].question}</p>
            </div>
            <div className="grid gap-3">
              {QUESTIONS[currentQuestion].options.map((option, index) => (
                <button key={index} onClick={() => handleAnswer(index)} className="group relative w-full text-left p-4 rounded-2xl border-2 border-slate-100 hover:border-indigo-500 hover:bg-indigo-50 transition-all flex items-center gap-4">
                  <span className="flex-shrink-0 w-10 h-10 rounded-xl bg-slate-50 flex items-center justify-center font-bold text-slate-400 group-hover:text-indigo-600 transition-colors uppercase text-sm">
        ...
        {gameState === 'RESULT' && (
          <div className="animate-in fade-in duration-500 max-h-[85vh] overflow-y-auto custom-scrollbar">
            <div className="p-8 text-center sticky top-0 bg-white z-10 border-b border-slate-50">
              {hasPassed ? (
                <div className="flex flex-col items-center">
                  <div className="bg-green-100 w-20 h-20 rounded-full flex items-center justify-center mb-4"><Trophy className="text-green-600 w-10 h-10" /></div>
                  <h2 className="text-2xl font-black text-slate-900">¡Increíble, {userName}!</h2>
                  <p className="text-green-600 font-bold text-xs uppercase tracking-widest mt-1">Amigo nivel Experto</p>
                </div>
              ) : (
                <div className="flex flex-col items-center">
                  <div className="bg-rose-100 w-20 h-20 rounded-full flex items-center justify-center mb-4"><XCircle className="text-rose-600 w-10 h-10" /></div>
                  <h2 className="text-2xl font-black text-slate-900">Uff, {userName}...</h2>
                  <p className="text-rose-600 font-bold text-xs uppercase tracking-widest mt-1">Necesitas ponerme más atención</p>
                </div>
              )}
            ...
            <div className="p-6 space-y-4">
              <button onClick={() => setShowReview(!showReview)} className="w-full flex items-center justify-between p-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-black transition-all">
                <div className="flex items-center gap-2"><Info className="w-4 h-4" /> {showReview ? 'Ocultar Revisión' : 'Ver en qué fallaste'}</div>
                {showReview ? <ChevronUp className="w-5 h-5" /> : <ChevronDown className="w-5 h-5" />}
              </button>

              {showReview && (
                <div className="space-y-3 animate-in slide-in-from-top-4 pb-4">
                  <h3 className="text-sm font-black text-slate-400 uppercase px-2 mb-4 text-left">Detalle de respuestas:</h3>
                  {userAnswers.map((ans, idx) => (
                    <div key={idx} className={`p-4 rounded-2xl border text-left ${ans.isCorrect ? 'bg-green-50 border-green-100' : 'bg-rose-50 border-rose-100'}`}>
                      <div className="flex gap-3">
                        <div className={`mt-1 flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center ${ans.isCorrect ? 'bg-green-500' : 'bg-rose-500'}`}>
                        ...
                        <div className="flex-1">
                          ...
                          <div className="grid grid-cols-1 gap-1 text-xs">
                            <p className={`${ans.isCorrect ? 'text-green-700' : 'text-rose-700'} font-medium`}>Tu respuesta: <span className="font-bold underline">{ans.selectedLabel}</span></p>
                            {!ans.isCorrect && <p className="text-slate-600 font-medium">Respuesta correcta: <span className="text-green-600 font-bold">{ans.correctLabel}</span></p>}
                          </div>
                        </div>
                      </div>
                    </div>
              ...
              <button onClick={resetQuiz} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-4 rounded-2xl transition-all flex items-center justify-center gap-2 shadow-lg">
                <RefreshCw className="w-5 h-5" /> Volver al Inicio
              </button>
            </div>
          </div>
        )}
